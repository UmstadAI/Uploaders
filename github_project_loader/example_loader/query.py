...
def query_docs(query):
    ...
    chat = ChatOpenAI(openai_api_key=openai.api_key, model="gpt-4")

    embed = OpenAIEmbeddings(model=embed_model, openai_api_key=openai.api_key)

    pinecone.init(api_key=pinecone_api_key, environment=pinecone_env)
    index = pinecone.Index("zkappumstad-codebase")
...
result = query_docs("Can you write simple mina zkapp smart contract")
print(result)
